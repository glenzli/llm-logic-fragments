...
即所有在复合伪度量 $d_J$ 下与映射目标 $y$ 完全等价的输入点集合。$\{\mathfrak{F}_{\phi, J}(y)\}_{y \in \mathrm{Im}(\phi)}$ 构成 $\mathrm{dom}(\phi)$ 的严格等价类划分。当 $J = \{j\}$ 时，记单分量纤维为 $\mathfrak{F}_{\phi, d_j}(y)$。由 $d_J = \max_{j \in J} d_j$ 的定义直接得 $\mathfrak{F}_{\phi, J}(y) = \bigcap_{j \in J} \mathfrak{F}_{\phi, d_j}(y)$。
...
**证明**：记 $M = \min_{l \in I} [\mathbf{A}(\phi_1)]_{li}$。若 $M = 0$，由伪度量非负性结论平凡成立。下设 $M > 0$（允许 $M = +\infty$），并任取非负实数 $0 \leq r < M$。
对任意合法基点 $y \in \mathrm{dom}(\phi_2 \circ \phi_1)$，及任意扰动点 $x \in B_{d_i}(y, r) \cap \mathrm{dom}(\phi_2 \circ \phi_1)$：由于 $\mathrm{dom}(\phi_2 \circ \phi_1) \subseteq \mathrm{dom}(\phi_1)$ 使得局部吸收下界依然生效，且 $r < [\mathbf{A}(\phi_1)]_{li} \leq \alpha_{\phi_1, d_i, d_l}(y)$ 对所有 $l \in I$ 严格成立，依据吸收半径定义，$\phi_1$ 必在全分量上输出零极差：$d_l(\phi_1(x), \phi_1(y)) = 0$（$\forall l \in I$）。

由于 $x, y$ 的像保证落在后续算子定义域内（即 $\phi_1(x), \phi_1(y) \in \mathrm{dom}(\phi_2)$），选取 $\phi_2$ 的任意合法 Lip 矩阵 $\mathbf{L}^{(2)}$，利用 $\bar{\mathbb{R}}_+$ 算术约定（$0 \cdot \infty = 0$）展开放缩：
$$d_j(\phi_2(\phi_1(x)), \phi_2(\phi_1(y))) \leq \sum_{l \in I} L_{l \to j}^{(2)} \cdot d_l(\phi_1(x), \phi_1(y)) = \sum_{l \in I} L_{l \to j}^{(2)} \cdot 0 = 0$$

此即表明 $d_i(x,y) < r \implies d_j(\phi_2(\phi_1(x)), \phi_2(\phi_1(y))) = 0$。根据局部吸收半径上确界属性，必有 $\alpha_{\phi_2 \circ \phi_1, d_i, d_j}(y) \geq r$。对 $y$ 取全域下确界得 $[\mathbf{A}(\phi_2 \circ \phi_1)]_{ji} \geq r$。令逼近参数 $r \uparrow M$，即得全局代数下界。$\square$
...
> **注（纤维膨胀的链式可加性）**：由度量熵项的直接加减可知，算子链的纤维膨胀严格满足分步可加性：当级联满足定义域无损耗时，$\mathrm{FI}_{\varepsilon, d}(c_\phi) = \sum_{m=1}^{k} \mathrm{FI}_{\varepsilon, d}(\phi_m\big|_{\mathrm{Im}(\phi_{m-1} \circ \cdots \circ \phi_1)})$。即链越长，累积的纤维拓扑坍缩越深。
...
若前置算子 $\phi_1$ 的像点测距使 $y$ 与 $y'$ 落入后续算子 $\phi_2$ 关于 $d_j$ 的同一纤维（即 $y' \in \mathfrak{F}_{\phi_2, d_j}(\phi_2(y))$），则依据纤维定义，其后续输出差异 $d_j(\phi_2(y), \phi_2(y'))$ **被精确截断为零**，无论前步测距 $d_i(y, y')$ 量值如何，该极差终止向后续系统传播。

> **注（吸收半径作为截断的充分条件）**：若 $d_i(y, y') \leq \alpha_{\phi_2, d_i, d_j}(y)$，则由吸收半径定义有 $y' \in B_{d_i}(y, \alpha) \subseteq \mathfrak{F}_{\phi_2, d_j}(\phi_2(y))$，纤维截断的前提自动满足。因此，吸收半径给出了纤维截断发生的**可计算充分条件**。
...
**命题 3.4（截断迭代律）**：对于第 $m+1$ 步算子 $\phi_{m+1}$，任意输出分量 $d_j$ 下的像点测距满足：
...
**命题 3.5（纤维冲突的固有偏离下界）**：设复合算子链 $\hat{\psi} = \phi_k \circ \cdots \circ \phi_1$ 与一般算子 $\psi$ 共定义域。若 $\hat{\psi}$ 在内部传导的第 $m$ 步与 $\psi$ 发生纤维冲突，即存在 $x, x' \in \mathrm{dom}(\hat{\psi}) \cap \mathrm{dom}(\psi)$ 使得 $d_j(\phi_m(x_{m-1}), \phi_m(x'_{m-1})) = 0$（其中 $x_{m-1}, x'_{m-1}$ 为 $x, x'$ 经前 $m-1$ 步的中间态），则这两个算子在 $x$ 与 $x'$ 上的输出测距必然满足极小下界：
