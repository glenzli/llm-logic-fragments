...
**语言**：模型对长尾知识或相近概念的混淆，不因训练数据或参数量增加而消失。更深刻的事实是：**更大的模型学会了更多概念（$N$ 随规模上升），而嵌入维度 $d$ 没有等比增长，混叠地板因此升高**。规模既是治疗手段，也是病因——你用更大的模型压低了每个原语的逼近误差（$\varepsilon_i \to 0$），却同时因为知道了更多东西（$N \uparrow$）而加深了概念之间的强制折叠。
...
**两种失效的因果结构截然不同**：
- **路由混叠 = 门口犯错**：激活了错误的原语 $r_j$，后续 Value 向量执行得再精确也是在做错事。即使 $d$ 足够大（执行层无污染），门口选错了一切白搭。
- **执行混叠 = 执行时污染**：门口选对了正确的原语 $r_i$，但 $W_V$ 的方向空间容不下所有原语正交，输出里混入了 $r_j$ 的成分。即使路由完美（$d_k$ 足够），执行层的污染仍然存在。

这意味着两种幻觉在行为表现上不同：路由混叠产生**整体性错误**（答错了问题），执行混叠产生**内容性污染**（回答了正确的问题但细节失真）。
...
**推论 11.3e-补（$M^*$ 是移动靶）**：上述静态定义假设 $N$ 和 $d$ 固定。但实际上 $N$ 是模型和数据规模的单调函数——更大的训练集和更大的模型使更多概念被学入 $R_{\text{tr}}$（$N \uparrow$）。因此 $M^*$ 并非一个固定的终点，而是一个**随 $M$ 本身右移的移动靶**：

$$M^*(M) \approx \min\{M' : \varepsilon_{\max}(M') \leq c_{ij}^*(d, N(M')) \cdot \|v^*\|\}$$

当 $N(M)$ 的增速快于 $d$ 的增速时（通常如此），$M^*$ 会持续右移——你永远无法通过单纯扩参数量「追上」混叠地板。只有让 $d$ 的增速跟上 $N(M)$ 的增速，或者主动限制 $N_\text{eff}$（RAG），才能真正出逃。

> **Scaling Law 饱和的 IDFC 机制根因**：实验观察到的训练 Scaling 边际收益曲线弯折，在 IDFC 中的精确解释是：$M$ 增大使 $\varepsilon_{\max}(M)$ 逼近 $c_{ij}^* \cdot \|v^*\|$ 后，继续 Scaling $M$ 压低的是已不再是瓶颈的 UAT 精度项，而非控制地板的混叠项——而且 $M$ 的增大还在同步推高 $N(M)$，使地板本身上移。**这是扩展 $d$（嵌入维度）与扩展 $M$（参数量）在 Scaling 阶段必须同步的 IDFC 理论依据，也是 o1/o3 系列转向 TTC（推断时 $l_{\max}$ 扩展）的另一角度解读：在 $M$ 已过 $M^*$ 后，增加推断时计算比增加训练参数更有效（§18.5 命题）。**
...
> | 幻觉分类 | 11.3e-补 | 严格 | $M^*$ 是移动靶：$N$ 是 $M$ 的单调函数（更大训练集/模型使更多概念进入 $R_{\text{tr}}$），故 $M^*$ 随 $M$ 本身右移；$N(M)$ 增速快于 $d$ 时永远无法单靠扩参数量「追上」混叠地板 |
