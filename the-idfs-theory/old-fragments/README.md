...
| **Part 2 数学基础**（按主题拆分） | | |
| ↳ [**part2a-model-proof.md**](./part2a-model-proof.md) | 数学建模 + CAC 定理证明 | §1 形式定义（$\mathcal{X}$、$\Omega$、$F$、$f$-chain）· §2 CAC 假说 · §3 完整证明（Telescope 展开 + UAT + TSC 对偶性） |
| ↳ [**part2b-fspace-morphology.md**](./part2b-fspace-morphology.md) | $F$-空间形态学 | §25 知识 / 逻辑 / 逐字记忆三态的形式定义 · Lipschitz 一致性分类 · 知识-逻辑纠缠命题 |
| **Part 3 推论**（按主题拆分） | | |
...
阅读顺序：`part1` → `part2-foundations` → `part2a` → `part2b` → `part3a` → `part3b` → `part3c` → `part3d` → `part4a` → `part4b` → `part4c` → `part4d`。
