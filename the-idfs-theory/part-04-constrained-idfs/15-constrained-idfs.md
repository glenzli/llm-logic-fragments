...
§3 的 CAC 定理给出了宏观误差的**上界**——系统的端到端误差不超过 $(\varepsilon_{\max}+\rho_{\max}+\Delta_{\max}) \cdot \Lambda_l + \delta_{\max} \cdot \Gamma_l$。§4 的 CAB 定理给出了误差的**下界**——对任何 IDFS，当目标变分足够大时，$\varepsilon^*_y \geq |\Delta - \varepsilon_x| - \Omega_l(\delta)$ 无条件成立。
...
>
> CAB（§4.1）作为定理对任何 IDFS 无条件成立——它不是假设，而是系统必须面对的物理现实。当目标具有足够大的变分（$\Delta > \varepsilon_x + \tau$，即任务"有趣"到不能被拟合噪声和容差掩盖）时，$\tau$-可控性要求与 CAB 的下界联立，产生对 $\Theta_{1,l}$ 的双向夹击。
>
...
- 若 $\bar{L}_q$ 击穿走廊上界，则 CAC 上界 $(\varepsilon_{max}+\rho_{max}+\Delta_{max})\Lambda_l + \delta_{max}\Gamma_l > \tau$，系统丧失 $\tau$-可控性的保证；
...
> **注（边界脆弱性）**：设 $x_0 \in \partial\mathcal{X}_{safe}$（安全域与失控域的边界），定义对抗扰动半径 $\delta_{adv} \triangleq \inf\{ d(x_0, x') : x' \in \mathcal{X}_{fail} \}$。由 $\partial\mathcal{X}_{safe}$ 的定义，$\delta_{adv}$ 可任意小——安全域边界附近总存在距离任意近的失控域点。走廊越窄（目标变分跨度越大时走廊越紧），安全域与失控域的交界面越密集，$\delta_{adv}$ 在更大范围内趋零。这意味着 Type III 系统具有**结构性的、与训练数据无关的**对抗脆弱性——攻击者无需理解系统的语义功能，只需找到使 $\bar{L}_q$ 跨越走廊边界的微小输入偏移。

**推论 7.19（失控域测度下界与有效容量折损，Fail Domain Measure Bound and Effective Capacity Loss）**：设 $L < \infty$。定义**走廊兼容路径集** $\mathcal{P}_{safe} \subseteq \mathrm{Im}(\sigma_l)$ 为所有 $\Theta_{1,l}$ 落在命题 7.1 走廊内的路径，$\mathcal{P}_{fail} = \mathrm{Im}(\sigma_l) \setminus \mathcal{P}_{safe}$ 为走廊外路径。定义**走廊效率** $\kappa_{safe} = |\mathcal{P}_{safe}| / |\mathrm{Im}(\sigma_l)| < 1$。则：

(i) **失控域测度下界**：$\mu(\mathcal{X}_{fail}) \geq \mu(\sigma_l^{-1}(\mathcal{P}_{fail}))$。走廊外路径的全部输入前像皆为失控域点，其测度严格为正（由命题 7.18(iii)）。

(ii) **有效容量折损**：仅安全路径 $\mathcal{P}_{safe}$ 能为目标集 $R$ 提供 $\tau$-精度服务。由 §4.2 CPI 定理，安全路径必须独立满足目标覆盖需求：

$$|\mathcal{P}_{safe}| \;\geq\; \frac{\mathcal{N}(A,\; 2\varepsilon/k)}{\mathcal{N}(A,\; \varepsilon/L)}$$

从而总路由容量需膨胀 $1/\kappa_{safe}$ 倍：

$$|\mathrm{Im}(\sigma_l)| \;\geq\; \frac{1}{\kappa_{safe}} \cdot \frac{\mathcal{N}(A,\; 2\varepsilon/k)}{\mathcal{N}(A,\; \varepsilon/L)}$$

**证明**：

(i) 走廊外路径 $q \in \mathcal{P}_{fail}$ 的 $\Theta_{1,l}(q)$ 违反命题 7.1 的走廊约束。违反下界时——$\Theta_{1,l} < D / (\delta + \Delta_{max}S_l)$——CAB 下界给出 $\varepsilon^*_y \geq |\Delta - \varepsilon_x| - \Omega_l(\delta) > 0$，误差有不可压缩的正下界，$\tau$-可控性**严格不可能**。违反上界时——$(\varepsilon_{max}+\rho_{max}+\Delta_{max})\Lambda_l + \delta_{max}\Gamma_l > \tau$——CAC 上界不再封顶于 $\tau$，系统**丧失** $\tau$-可控性的保证。无论哪种情况，被路由至 $q$ 的输入均不具备 $\tau$-可控的确定性保障，故 $\sigma_l^{-1}(\mathcal{P}_{fail}) \subseteq \mathcal{X}_{fail}$。$\mu(\sigma_l^{-1}(\mathcal{P}_{fail})) > 0$ 由命题 7.18(iii) 保证。

(ii) 仅走廊兼容路径 $\mathcal{P}_{safe}$ 能确定性地为目标集 $R$ 提供 $\tau$-精度服务。CPI 的目标覆盖需求必须由 $\mathcal{P}_{safe}$ 单独承担。由 $|\mathcal{P}_{safe}| = \kappa_{safe} \cdot |\mathrm{Im}(\sigma_l)|$，CPI 下界对 $|\mathrm{Im}(\sigma_l)|$ 产生 $1/\kappa_{safe}$ 倍的膨胀因子。$\square$

> **注（三种构型的对偶结构）**：推论 7.19 补全了三种构型的对偶量化格局。Type I 的代价是采样规模下界（推论 7.15）——路由简并迫使 $M$ 直接承担覆盖数。Type II 的代价是空间代偿（推论 7.17）——深度截断迫使 $M$ 指数膨胀。Type III 的代价是容量折损——失控域"浪费"了 $1 - \kappa_{safe}$ 比例的路由容量，迫使总容量以 $1/\kappa_{safe}$ 倍膨胀来代偿。三种代价的共同根源是路径乘积走廊对系统自由度的刚性约束。
...
综合命题 7.18 与推论 7.19：Type III 系统保留了对高变分目标的服务能力和长链深度，但不可避免地产生正测度的失控域——其内误差可达像空间直径级别，且 $1 - \kappa_{safe}$ 比例的路由容量被浪费。安全域与失控域的边界由走廊宽度决定，形成极薄的脆弱剖面，微小扰动即可触发灾难性跳变。

$\to$ **画像：脆界放大器**。Type III 在安全域内性能优异（§7.1 全部生效），但失控域的存在不仅不可消除（测度正下界），其边界更是由走廊宽度决定的**薄壳脆弱剖面**。目标变分跨度越大，走廊越窄，安全域与失控域的交界面越脆弱。

> **注（失控域误差发散的微观机制：采样域劫持，与 命题 5.3 的对接）**：上述命题 (ii) 从宏观层面证明了失控域内误差可达像空间直径 $D$，但未揭示其微观发生机制。命题 5.3（组合误差强制放大）恰好填补了这个缺口。当系统在走廊边界附近工作时（边界脆弱性注记的 $\delta_{adv}$ 极小），微小输入扰动将中间态 $h_j$ 推入一个由**不相关采样约束**锁定行为的区域——$\Phi$ 在该区域的全局行为已被另一条采样对 $(r', \mathcal{X}')$ 强制绑定为服务于 $r'$，无法为当前链路 $q$ 提供正确的映射。误差发散不是因为"系统没学好"（$\varepsilon$ 在每条规则上均可任意小），而是因为"系统在那个区域学的是别的东西"——这是 IDFS 单映射共享架构的固有副作用。
>
> 由此，Type III 的失败模式具有**离散跳变型（catastrophic）**而非连续退化型（graceful）的特征：误差从安全域的 $B_{sat}$ 跳变至失控域的 $\Omega(D)$，跳变幅度不低于 $2 \cdot d(\text{劫持域中心}, \text{目标值域})$（命题 5.3 的 $2D$ 下界），远非 $O(\delta_{adv})$ 级别的连续退化。这一离散跳变性质与边界脆弱性注记联合，揭示了 Type III 系统的一个严峻工程现实：**失败不是逐渐发生的——系统在走廊边界的一侧表现完美，跨过微小的 $\delta_{adv}$ 后立即灾难性崩溃，中间没有预警的渐变区间。**
...
> | 参数代偿 | 无需额外代偿（$M$ 未被利用） | $M$ 指数膨胀（推论 7.17，空间换时间） | 容量折损 $1/\kappa_{safe}$（推论 7.19） |
...
- **Type III 违反边界 1**：由 §7.2.3 双峰不可消除性命题，系统必然存在 $\mu(\mathcal{X}_{fail}) > 0$ 的失控域，其内误差可达 $D \gg \tau$。由推论 7.19，有效路由容量因走廊效率 $\kappa_{safe} < 1$ 而折损，失控域不可消除。由边界脆弱性注记，$\delta_{adv}$ 极小。
...
**推论 7.22（长链复合的架构必然性，Architectural Necessity of Long-Chain Composition）**——设 IDFS $\mathcal{F} = (F, \sigma)$（$L < \infty$）为 Type II 系统（$l \le l^*_0$），需完成等效链深 $l_{eff} \gg l^*_0$ 的复合任务且要求全域有界性（$\forall x,\; \varepsilon(x) \le \tau$）。则：

(i) **自迭代退化**：级联系统 $\mathcal{F}^{l_{eff}}$（$\Phi$ 的 $l_{eff}$ 次自复合）满足 $l_{eff} > l^*_0$。若要求 $\mathcal{F}^{l_{eff}}$ 维持 $\tau$-可控性，则推论 7.2（长链收缩必要性）生效，$\Theta_{1,l_{eff}} \to 0$，系统陷入定理 7.8 的三重缺陷。若放弃 $\tau$-可控性，则由命题 7.18，$\mu(\mathcal{X}_{fail}) > 0$——系统退化为 Type III。

(ii) **段间外部干预的必要性**：由 (i)，不引入额外机制的自迭代必然失去 Type II 的全域有界性。为保持 $\forall x,\; \varepsilon(x) \le \tau$，系统必须将 $l_{eff}$ 步级联分解为段长 $\leq l^*_0$ 的片段，并在段间施加某种**外部误差缩减算子** $\psi: \mathcal{X} \to \mathcal{X}$（外部于 $\Phi$）。$\psi$ 不要求完全消除段间累积误差——设段间残余误差为 $\varepsilon_{reset} \triangleq d(\psi(h_k), x^*_{k+1})$，则只要 $\varepsilon_{reset}$ 受控，后续段即可从受控的初始偏差重新启动，累积误差不越过 $\tau$。有效深度 $l_{eff}$ 是 $\psi$ 的**干预频率**（段长 $l_0$）与**缩减质量**（$\varepsilon_{reset}$ 的大小）的连续函数：$\varepsilon_{reset}$ 越小，段间累积越慢，可支撑的 $l_{eff}$ 越大。

(iii) **双层结构的必然性**：由 (i)–(ii)，$l_{eff}$ 深度任务在全域有界性约束下的实现形式必然包含两个不可化约的层次：**IDFS 内核**（每段 $\Phi^{l_0}$，$l_0 \leq l^*_0$，负责段内精确映射）与**外部控制环**（负责段调度与段间误差缩减 $\psi$）。系统的最大有效深度 $l_{eff}$ 不受 $l^*_0$ 限制，而受外部控制环的干预频率和 $\psi$ 的缩减质量限制。

**证明**：

(i) $\mathcal{F}^{l_{eff}}$ 是 $\Phi$ 的 $l_{eff}$ 次级联，$l_{eff} > l^*_0$。由推论 7.2，$\tau$-可控性蕴含 $\Theta_{1,l_{eff}} \to 0$；由定理 7.8，系统遭遇路径乘积消亡、输入致盲、拟合瓶颈三重缺陷。若放弃 $\tau$-可控性（即允许 $\exists x,\; \varepsilon(x) > \tau$），则系统不再要求所有路径满足走廊约束，走廊外路径被自然保留；由命题 7.18(iii)，这些走廊外路径的输入前像构成正测度失控域 $\mathcal{X}_{fail}$。两条分支均丧失 Type II 性质。

(ii) 由 (i)，任何保型方案都必须将 $l_{eff}$ 步分解为段长 $\leq l^*_0$ 的片段（否则触发退化）。段间状态传递若为恒等映射（$\psi = \mathrm{id}$），则与原始自迭代无异，仍触发 (i)；因此 $\psi \neq \mathrm{id}$ 是必要条件。此外，$\psi$ 必须外部于 $\Phi$——若 $\psi$ 本身是 $\Phi$ 的进一步迭代，则等效链深增加，仍触发 (i)。$\psi$ 的缩减效果由 $\varepsilon_{reset}$ 量化：$\varepsilon_{reset} = 0$ 时段间误差被完全消除（最强干预），$\varepsilon_{reset} > 0$ 时残余误差进入后续段的初始偏差，通过段内 Lipschitz 传播 $L^{l_0}$ 放大。当 $L^{l_0} < 1$（段内收缩主导）时，即使 $\varepsilon_{reset} > 0$，段间误差递推仍收敛至有限饱和值；当 $L^{l_0} \geq 1$ 时，$\varepsilon_{reset}$ 必须足够小以防止发散。

(iii) 由 (i)–(ii) 直接得。$\square$

> **注（Type II 的抗劫持鲁棒性：构型优选的第三条论证，与 命题 5.3–4 的对接）**：命题 5.4 证明了在固定系统容量下，采样集 $\mathcal{S}$ 的扩增使最优可达拟合效果**单调非递增**——特别是模式 B（新采样对追加）的净效果恒为负，因为新约束通过 命题 5.3 的劫持机制锁死 $\Phi$ 在中间态域的行为。这为 Type II 的构型唯一性在误差全域有界性和采样非指数爆炸两个边界之外提供了**第三条独立论证**：
>
> - **Type I 受劫持恶化**：Type I 的全域穷举策略要求 $|\mathcal{S}|$ 极大。每追加一批采样对，命题 5.4 模式 B 保证可行集进一步收缩，且新采样在域外的锁定效应通过劫持机制**结构性地恶化**已有链路的性能——这不仅是维度诅咒的采样代价，更是采样本身的**自毒化效应**。
> - **Type III 受劫持剧烈**：由上方注记，失控域 $\mathcal{X}_{fail}$ 的误差发散恰恰由劫持机制驱动。$|\mathcal{S}|$ 越大，中间态轨道途经"已被其他采样对锁定"的区域的概率越高，劫持发生得越频繁，失控域的实际危害越严重。
> - **Type II 天然抗劫持**：链深 $l \le l^*_0$ 极短，中间态 $h_j$ 偏离初始采样域的累积漂移有限（$\delta$ 增长被截断在 $E \cdot l^*_0 = \tau$ 以内），被不相关采样对劫持的几率受到结构性抑制。更关键的是，分段复合架构中的段间外部干预 $\psi$ 将中间态强制拉回受控起点，从根本上阻止了中间态漂移进入劫持域。

> **注（外部干预的反劫持解释，与 命题 5.3 的深层对接）**：推论 7.22 从代数层面证明了段间外部干预 $\psi$ 截断 $\Theta_{1,l}$ 的乘性累积。但从 §5 的视角审视，$\psi$ 的功能远不止截断乘性传播——它更根本地是一个**反劫持机制**。在无干预的连续长链中，中间态 $h_j$ 的累积漂移使其逐步远离初始采样域，不可避免地进入由其他采样对锁定的"他方领地"（命题 5.3 的劫持域）。$\psi$ 通过在每段终点将中间态拉回受控起点来**强制阻止链路穿越劫持域的边界**。因此，$\psi$ 的功能在度量空间层面是"缩减误差"，但在 $\Phi$ 的行为空间层面是"将轨线拉回安全领地"。这两个解释互补而非重复：前者是定量的（误差界），后者是定性的（行为安全性）。

----
*[IDFS] ⊢ [GLENZLI] ⊢ [Part 02] ⊢ [07-constrained-idfs] ⊢ [727f3e2491ae7540]*
