...
**命题 2.8（扰动的长链衰减，Long-chain Decay of Perturbations）**：在级联系统 $\mathcal{F}^l$ 中，设 $\hat{h}^{(A)}$、$\hat{h}^{(B)}$ 为两条从不同初始点出发的 $\Phi$-轨道。定义第 $j$ 步的**有效扩张比** $L_j \triangleq d(\hat{h}_j^{(A)}, \hat{h}_j^{(B)}) / d(\hat{h}_{j-1}^{(A)}, \hat{h}_{j-1}^{(B)})$（$\leq L$，由 $\Phi \in \mathrm{Lip}_L$ 保证——无论两轨道在第 $j$ 步是否被路由到同一激活链）。设第 $k-1$ 步存在任意来源的**状态扰动** $\Delta_{k-1} \triangleq d(\hat{h}_{k-1}^{(A)}, \hat{h}_{k-1}^{(B)}) > 0$。若**尾部乘积** $\Theta_{k,l} \triangleq \prod_{j=k}^l L_j \to 0$（$l \to \infty$，即收缩步主导），则：
