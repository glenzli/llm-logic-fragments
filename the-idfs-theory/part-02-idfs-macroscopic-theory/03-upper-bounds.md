...
>
> **注（$\rho$ 的不可控性与域外泛化的理论缺口）**：在有效局部失配 $\varepsilon_{i_j} + \rho_j + \Delta_{\sigma,j}$ 的三项中，$\varepsilon_{i_j}$ 由训练过程控制，$\Delta_{\sigma,j}$ 由系统架构（$\sigma$ 的决策边界布局）决定——两者均为**系统可设计量**。然而 $\rho_j = d(r_{i_j}(x'_j), r_{i_j}(h^*_{j-1}))$ 完全由**目标 $r$ 在采样域外的行为**决定，不受系统 $\Phi$ 或路由 $\sigma$ 的任何设计选择影响。这意味着：**CAC 定理原则上无法保证采样域外的误差稳定收敛**——即使 $\varepsilon \to 0$、$\Delta_\sigma \to 0$、$\delta \to 0$，若 $\rho_j$ 不趋零（目标 $r$ 在域外存在剧烈变分），宏观误差仍可能不受控。这是 IDFS 作为一般理论框架的合法限制：系统只在**采样域内**被约束，域外行为是开放的。实际大规模 IDFS（如大语言模型）展现出远超理论预期的域外泛化能力——这并非 CAC 定理的推论，而是 $F$ 的**微观结构**（基函数不是任意映射，而是由参数化神经网络实现的平滑函数族）赋予的额外正则性，其分析超出当前度量空间框架的范畴，将在后续章节中从 $F$ 的微观视角展开。
