...
>
> **注（$\Delta_{\sigma}$ 与 命题 2.7 的关系）**：命题 2.7 证明了路由分辨率极限 $d(a,b) \geq \Delta/L$。$\Delta^{err}_j$ 正是该命题的定量后果：当累积误差 $e_{j-1}$ 超过理想轨道到最近路由边界的距离 $d(h^*_{j-1}, \partial\mathrm{Link})$ 时（§2.4 所述的分辨率死锁临界），$\sigma$ 不可避免地将实际状态分配到**不同激活链**，$\Delta^{err}_j > 0$ 随即被触发。命题 2.4 所揭示的离散曲率爆发（接近边界时 $L_j \to L$）是 $\Delta^{err}_j$ 的**连续面预兆**；$\Delta^{err}_j$ 本身是**跨越边界后的离散面后果**。二者合成了路由边界的完整双重危害。
