...
>
> **注（$\Delta_{\sigma}$ 与 命题 2.14 的关系）**：命题 2.14 证明了路由分辨率极限 $d(a,b) \geq \Delta/L$。$\Delta^{err}_j$ 正是该命题的定量后果：当累积误差 $e_{j-1}$ 超过理想轨道到最近路由边界的距离 $d(h^*_{j-1}, \partial\mathrm{Link})$ 时（§2.5 所述的分辨率死锁临界），$\sigma$ 不可避免地将实际状态分配到**不同激活链**，$\Delta^{err}_j > 0$ 随即被触发。命题 2.14 所揭示的离散曲率爆发（接近边界时 $L_j \to L$）是 $\Delta^{err}_j$ 的**连续面预兆**；$\Delta^{err}_j$ 本身是**跨越边界后的离散面后果**。二者合成了路由边界的完整双重危害。
...
> - **(I') 误差致路由偏转**（$= \Delta^{err}_j$）：累积误差将实际状态推过路由边界，导致系统调用了"错误的"激活链。这是 命题 2.14（路由分辨率极限）在误差链中的直接后果——当 $e_{j-1} > d(h^*_{j-1}, \partial\mathrm{Link})$ 时，路由跳变不可避免。
...
由于 IDFS 是有限系统（有限步长 $l$、有限全局常数 $L$、有限单步误差 $\varepsilon_{max}$、有限偏离 $\delta_{max}$），且 $\Phi^l$ 自身仍然是满足 $L^l$ 约束的 IDFS（§1.2 代数自相似性），宏观容差上界 $\varepsilon^*_q$ **恒为有限数**。更重要的是，由于 命题 2.14（路由分辨率极限）的绝对物理约束，当误差积累使轨线间距被压缩到 $\sigma$ 的分辨率死锁以下时，系统必然发生**路径合并**——无论 $\bar{L}$ 多大，持续扩张的链路在物理上无法维持，误差增长被系统的有限容量自动截断。因此，从微观容差 $\mathcal{E}$ 跃迁至宏观容差 $\mathcal{E}^*$ 呈现两种物理情形：
...
> **注（$l \to \infty$ 极限的合法性）**：虽然 IDFS 是有限系统，但此处取 $l \to \infty$ 在物理上是合法的。推论 3.6 的前提条件要求收缩步主导（$\Theta_{j,l} \to 0$），而收缩路径不触发任何物理截断机制——系统只是将轨线越拉越近，不涉及 命题 2.14 的分辨率死锁或路径合并（后者仅约束扩张行为）。因此，无限长收缩链的极限分析在 IDFS 框架内是完全自洽的。
