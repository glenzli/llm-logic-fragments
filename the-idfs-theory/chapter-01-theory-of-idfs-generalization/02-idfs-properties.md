...
>
> 这一概念与 §2.6 命题 6 的路径 Lipschitz 跨度 $\kappa_\Phi$（§1.2 定义）深刻关联：
...
§2.5 揭示了 IDFS 迭代映射的动力学行为：收缩尾积压平扰动（命题 3），扩张尾积放大扰动（命题 4），$\sigma$-边界更产生局部极其剧烈且受制于分辨率极限的最大扩张差 $L_j$（命题 5）。这自然引出一个系统设计层面的问题：系统在长链中所展现出的**路径曲率**必须有多大的变异范围？本节表明，这一变异范围由目标任务集的多样性从下方强制决定。
...
**推论 7（动态张力的阿喀琉斯之踵：能力与脆弱性的结构同源）**：结合第 3 章的 CAC 类紧致性定理（推论 3 及其同向共面拉伸构造），系统为了满足复杂任务环境所必须建立的宏观张力 $\kappa_\Phi$，直接构成了其遭遇分布外（OOD）扰动时发生坍缩的结构性命门。

**证明（同构性论证）**：由命题 6，当目标集逼近难度极高（要求极强的局部拉伸去区分概念，即 $\gamma_R \gg 1$），系统的生存必然依赖于在某些特征子空间内建立具有极大 Lipschitz 常数 $L_{max} \approx \sup L_j$ 的高能演化走廊。然而，在第三章的同向共线拉伸构造中，触发 CAC 残酷代数上限 $\mathcal{O}(L_{max}^l)$ 的充要条件，正是微观偏差与域外偏离 $\delta$ 顺着一条具备极大本征拉伸率 $L_j$ 的主特征射向发生同向累加。
