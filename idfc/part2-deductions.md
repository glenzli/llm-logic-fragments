...
> **前置**：本文建立在 [Part 1](part1-model.md) 的数学基础上（IDFC 框架、CAC 定理、误差界）。
> **本文内容**：§4 从 CAC 出发解释涌现、顿悟、CoT 有效性和能力聚簇；§5 建立与行为吸引子理论的连接；§6 列出开放问题。
> **后续**：[Part 3](part3-extensions.md) 对 CAC 进行更深层的形式推演；[Part 4](part4-attention.md) 给出可观测的 Attention 界。
...
> **命题 4.3（CoT 能力扩展）**：对可靠链路长度上限为 $l_{\max}$ 的模型，$k$ 步 CoT 将可解问题的有效 $r$-链深度从 $l_{\max}$ 扩展至 $k \cdot l_{\max}$，且误差保持可控。（含 $\varepsilon_{\text{tok}}$ 成本的精确版本见 [Part 3 §7.3](part3-extensions.md)）
