# IDFC · Part 3：CAC 扩展推论

> **前置**：本文建立在 [Part 1](part1-model.md) 的 CAC 误差界之上，对定理进行更深层的形式推演。每个命题明确标注证明状态：
> - **严格推论**（从现有定义直接可证）
> - **需额外假设的命题**（标注所需假设）
> - **开放猜想**（目前无证明路径）
>
> 本文内容：§7 能力边界；§8 计算不可约性；§9 对齐脠弱性；§10 认识论极限与逆定理猜想。
...
命题 4.3（[Part 2](part2-deductions.md) §4.4）给出了 CoT 的定性描述，此处给出含中间 token 生成误差的精确版本。
...
将 CAC 的误差结构与行为吸引子理论（[Part 2 §5](part2-deductions.md)）结合，可以得到对齐问题的最强形式结论。
