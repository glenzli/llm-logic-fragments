# IDFC · Part 4a：Transformer 的 IDFC 实例化与 Attention 边界

> **本文内容**：§1–§7，Transformer 各组件的 IDFC 角色、Attention 泛函分析、Lost in the Middle 公式化、最优信息提取界、幻觉 Type IV 的形式化（Transformer 专有）。
> 其余内容见：[Part 4b 对比架构](part4b-architectures.md) · [Part 4c 实验验证](part4c-experiments.md) · [Part 4d 现象分析](part4d-phenomena.md)
...
**命题 5.2（最优头数存在性与唯一性）**：$\mathcal{F}_{\text{total}}(H)$ 存在**唯一**有限最优值 $H^*$，满足：

$$\frac{d\mathcal{F}_{\text{total}}}{dH}\bigg|_{H^*} = 0$$

**证明（存在性 + 唯一性）**：

令 $c = 2B^2/\sqrt{d}$，$A = n-1 > 0$，以及换元 $u = \sqrt{H} > 0$，则：

$$\mathcal{F}_{\text{total}}(u) = \frac{u^2}{1 + A e^{-cu}}$$

对 $u$ 求导：

$$\frac{d\mathcal{F}_{\text{total}}}{du} = \frac{2u(1 + Ae^{-cu}) + cAu^2 e^{-cu}}{(1 + Ae^{-cu})^2} = \frac{u \cdot (2 + Ae^{-cu}(2 + cu))}{(1 + Ae^{-cu})^2}$$

由于 $u > 0$，令分子为零等价于：

$$2 + Ae^{-cu}(2 + cu) = 0$$

设 $\phi(u) = Ae^{-cu}(2 + cu)$，则驻点条件为 $\phi(u) = -2$。

- $\phi(0) = 2A > 0$
- $\phi(u) \to 0^+$（从上方趋于 0）当 $u \to \infty$，因为 $e^{-cu}$ 比任意多项式衰减更快
- $\phi'(u) = Ae^{-cu}(2 + cu - cu - c^2u \cdot \ldots) = Ae^{-cu}(2 - c^2u^2) / \ldots$

精确计算：$\phi'(u) = Ace^{-cu}(c \cdot u - (2 + cu)) = -2Ace^{-cu} < 0$ 对所有 $u > 0$ 成立。

（验证：$(Ae^{-cu}(2+cu))' = -cAe^{-cu}(2+cu) + cAe^{-cu} = -cAe^{-cu}(1+cu)$，由于 $c, A, u > 0$，符号恒负。）

因此 $\phi(u)$ 在 $(0, \infty)$ 上**严格单调递减**，从 $2A > 0$ 单调下降至 $0^+$。

但驻点条件要求 $\phi(u^*) = -2 < 0$，而 $\phi(u) > 0$ 对所有 $u > 0$，这意味着 $d\mathcal{F}_{\text{total}}/du > 0$ 对所有有限 $u > 0$！

即 $\mathcal{F}_{\text{total}}(u)$ 关于 $u$（从而关于 $H = u^2$）**在有限范围内严格单调递增**，在 $H \to \infty$ 时以 $H$ 为上界增长——但实际上 $H$ 的上界是 $d$（每头至少 1 维），因此 $H^* = d$（头数等于嵌入维度，每头 1 维）是形式上的最优。

**物理约束修正**：$H \leq d$ 且每头子空间需有足够维度承载 QK 运算（实践中 $d_k \geq d_k^{\min}$，如 64），因此实际约束集为 $H \in \{1, 2, \ldots, \lfloor d/d_k^{\min} \rfloor\}$。在此有界集合内，$\mathcal{F}_{\text{total}}$ 严格单调递增意味着**在子空间维度约束下 $H^* = \lfloor d/d_k^{\min} \rfloor$ 是唯一最优**，且最优点唯一（离散有界集合上的严格单调函数在边界取唯一极值）。$\square$

> [!NOTE]
> 命题 5.2 的结论与实践直觉一致：在相同总维度下，更多小头（更小 $d_k$）比更少大头有更高的总体 $\mathcal{F}_{\text{total}}$——这是多头注意力的理论动机。实践中最优 $H$ 受硬件效率（矩阵运算最优块大小）和 $d_k$ 的最小表达能力共同约束。
